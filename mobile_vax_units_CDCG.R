...
#only switch it on for the sensitivity test of excluding ethnicity
# model1<-svyglm(outcome2 ~ vacc_bus+age+sex+imd_score+chronic+carer+
#                  social_care+mdist_min, 
#                family=poisson(link=log), 
#                design=svydesign(ids=~pid, weights =~main, data=surv_data))
# summary(model1)
...
#only switch it on for the sensitivity test of excluding ethnicity
#write.csv(rr1, normalizePath(file.path(a,"main_result_no_ethnicity.csv")))
...
##################################  17. Summary statistics for the full C&M population ##########################

### generate summary statistics of the full C&M population
...
####################### 19. Sensitivity - excluding ethnicity#########################
...
####################### 21. Sensitivity - testing distance thresholds and spatial spill over effects #########################
#Simply switch on relevant lines of codes (as commented) to generate the results; 
#please refer to the separate script "mobile_vax_lookup_CDCG.R" for more details on how the lsoa lookup table is generated.


####################### 22. sensitivity - individual-level survival analysis  #########################
# construct survey data
...
####################### 23. calculate sample sizes for the subgroup analysis  #########################
...
subgrp_sz<-unique(subgrp_sz)


table(subgrp_sz$agegrp)
subgrp_sz[agegrp=="(65,200]",agegrp:="(65,110]"]
table(subgrp_sz$agegrp)


levels(subgrp_sz$ethn_f)
rev(levels(subgrp_sz$ethn_f))

table(subgrp_sz$ethn_f)
subgrp_sz[,ethn_f:=factor(ethn_f,levels=c("Asian or Asian British","Black or Black British","Mixed","Other Ethnic Groups","White"))]
table(subgrp_sz$ethn_f)
